...
import { currentValues, listVitolaOptions } from '@/lib/wiki/queries'
...
export default async function ProposePage({ params }: Params) {
  ...
  const [values, vitolas, open] = await Promise.all([
  ...
  return (
    <main id="contenu" className="mx-auto flex max-w-3xl flex-col gap-8 px-4 py-12">
      ...
      {open ? (
        <ProposeForm cigarId={cigar.id} slug={slug} current={current} vitolas={vitolas} />
      ) : (
